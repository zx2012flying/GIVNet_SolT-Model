...
TRAINING_FPATH = Path("vermeire/traindata_nonaq.csv")
...
def train_ensemble(*, data=None, remove_output=False, training_percent=None, **model_kwargs):
    ...
    if data is None:
        ...
        metadata_df_og = df[["solute_smiles", "solvent_smiles"]]
    ...
    if training_percent is not None:
        ...
        downsample_df = downsample_df.groupby(["solute_smiles", "solvent_smiles"]).aggregate(list)
    ...
    for replicate_number in range(NUM_REPLICATES):
        ...
        if SPLIT_TYPE == "source":
            ...
            metadata_df['solute_solvent_pair'] = metadata_df['solute_smiles'] + "_" + metadata_df['solvent_smiles']
            unique_pairs = pd.unique(metadata_df['solute_solvent_pair'])
            studies_train, studies_val = train_test_split(unique_pairs, random_state=random_seed, train_size=0.90,
                                                          test_size=0.10)
            train_indexes = metadata_df.index[metadata_df['solute_solvent_pair'].isin(studies_train)].tolist()
            val_indexes = metadata_df.index[metadata_df['solute_solvent_pair'].isin(studies_val)].tolist()
        ...
        tgrads = tgrads.groupby([ "solvent_smiles", "solute_smiles"])[["logS", "temperature", "source_index"]].aggregate(list)
...
def rename_recent_dir(updated_name):
    ...
    subdirs = [os.path.join(parent_dir, d) for d in os.listdir(parent_dir) if
    ...
    if os.path.exists(new_name):
        print(f"Target path {new_name} already exists. Please choose a different name.")
        return

    try:
        os.rename(most_recent_dir, new_name)
        print(f"Renamed {most_recent_dir} to {new_name}")
    except PermissionError as e:
        print(f"Permission error: {e}. Trying to use shutil.move instead.")
        try:
            shutil.move(most_recent_dir, new_name)
            print(f"Moved {most_recent_dir} to {new_name} using shutil.move")
