...
def parity_plot(truth, prediction, title, out_fpath, stat_str):
    ...
    plt.plot([min_val, max_val], [min_val + 0.6, max_val + 0.6], color="red", linestyle="--", alpha=0.25)
    plt.plot([min_val, max_val], [min_val - 0.6, max_val - 0.6], color="red", linestyle="--", alpha=0.25)
...
def test_ensemble(checkpoint_dir: Path):
    ...
    for holdout_fpath in (
        ...
        Path("vermeire/testdata_nonaq.csv"),
    ):
        ...
        wn_05 = np.count_nonzero(np.abs(out["logS_true"] - out["logS_pred"]) <= 0.5) / len(out["logS_pred"])
        wn_1 = np.count_nonzero(np.abs(out["logS_true"] - out["logS_pred"]) <= 1.0) / len(out["logS_pred"])
        r2 = r2_score(out["logS_true"], out["logS_pred"])
        stat_str = (
            f" R2: {r2:.4f}\n - MAE: {mae:.4f}\n - MSE: {mse:.4f}\n - RMSE: {rmse:.4f}\n - W/n 0.5: {wn_05:.4f}\n - W/n 1.0: {wn_1:.4f}"
        )
...
if __name__ == "__main__":
    ...
    for training_count in (20, 50, 100, 200, 500, 1000, 2000):
        testdata = test_ensemble(Path(f"output/fastprop_{training_count}/checkpoints"))
        # fastprop_sobolev_leeds_results.append([leeds_acetone, leeds_benzene, leeds_ethanol])
        fastprop_sobolev_solprop_results.append(testdata)
