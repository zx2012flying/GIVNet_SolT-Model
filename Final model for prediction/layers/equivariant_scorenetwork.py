...
class EquiLayer(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        out = self.propagate(edge_index.to(device) , x=x , edge_attr=edge_attr , size=size)
        return out
...
class EquivariantScoreNetwork(torch.nn.Module):
    ...
    def forward(self, edge_index, node_attr, edge_attr, equivariant_basis):
        ...
        for module_idx, gnn_layers in enumerate(self.gnn_layers):
            ...
            # generate gradient
            dynamic_coff = self.basis_mlp_modules[module_idx](edge_feature).to(device)  # (num_edge, 3)
            basis_mix = dynamic_coff[:, :1] * coord_diff.to(device)  + dynamic_coff[:, 1:2] * coord_cross.to(device)  + dynamic_coff[:, 2:3] * coord_vertical.to(device)   # (num_edge, 3)
